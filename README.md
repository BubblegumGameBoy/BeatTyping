...
```js
{
  ...
  composer: "W.A. モーツァルト",
  level: 2,
  bpm: 104,                // バッキングのグルーヴ速度（wait-mode では演奏を急かさない）
  meter: 2,                // 任意。4=4/4ポップ(既定) / 3=3/4ワルツ / 2=2/4マーチ
  tutorial: true,          // 任意。true なら maxLayer=0（オーケストラ無し）＋ホームポジ説明
  events: [
    // notes:   メロディー（押すと鳴る音）。和音は配列で複数
    // accomp:  伴奏の和音（root を先頭に）。ヒットの瞬間に弱いピアノ左手として鳴り、
    //          同時に backing clock の現在コードになる（tutorial でもピアノ左手は鳴る）
...
- レイヤー: combo≥8 で +cello+drums、≥20 で +violin、≥36 で +french horn。
  tutorial 曲は `maxLayer=0` で純ピアノ（左手伴奏あり・オーケストラ無し）。
- 演奏ダイナミクス: 打鍵間隔でベロシティとサスティンが変わる（速い連打＝軽く短く、
  ゆったり＝豊かに）。同じ音の連打は実ピアノ同様、前の発音をダンプしてから再打鍵。
  発音は `Tone.immediate()`（lookahead 無し）で打鍵に即応。
- オーケストラ: 弦パッドはコードが変わった瞬間に張り替え（小節頭まで古い和音を
  引きずらない）。チェロはコード root をオクターブ2に固定（サンプル音域内）、
  ホルンは root+5度のホルン五度。バッキングのパターンは `meter` に従う。
